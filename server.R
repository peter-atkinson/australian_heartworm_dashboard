function(input, output, session) {
  ...
  #create ui output main_page load a reactive UI depending on conditions
...
"
        ...
        column(9, 
               wellPanel(style = "height: 800px",
                 h3(
                   ...
                   leafletOutput("leaflet_chdu", height = 600, width =
                                   "100%")
                 )
               )),
        column(3, 
  ...
  output$selecteddatemap <- renderText({
    (paste(
      "As of",
      format(input$dates, format = "%d %b %Y"),
  ...
  output$leaflet_chdu <- renderLeaflet({
    chdu <-
      paste(
        #"C:/Users/a1667856/Box/PhD/HDU Mapping/hdu_mapping/hdumaps/", #local running
        "./hdumaps/", #WinSCP running
        "chdu",
        format(input$dates, format = "%Y%m%d"),
        ".tif",
        sep = ""
      )
    ...
    pal <-
      colorBin(
        c("royalblue3", "goldenrod2", "firebrick3"),
        bins = c(0, 120, 130, Inf),
  ...
  locationplotdata <- reactive({
    ...
    trial <-
      data.frame(dseq, {
        if (length(all_of(z)) != 0)
          dplyr::select(postcodes.all, (all_of(z)))
        else
          return(NULL)
      })
    ...
    trial[, 7] <-
      str_c(get_fy(trial[, 1], offset_period = -1), "/", get_fy(trial[, 1]))
    trial[, 8] <- day_of_year(trial[, 1], type = "financial")
    
    yearbreaks <-
      seq((as.numeric(format(
        min(dseq), format = "%Y"
      )) + 0.5),
      (as.numeric(format(
        max(dseq), format = "%Y"
    ...
    colours <-
      c(
        "Transmission season" = "firebrick3",
        "Transmission unlikely" = "royalblue3",
        "Shoulder" = "goldenrod2"
      )
    
    years <-
      seq(as.numeric(format(min(dseq), format = "%Y")), as.numeric(format(max(dseq), format =
                                                                            "%Y")), by = 1)
  ...
  statusdata <- reactive({
    ...
    todaystatus.df[, 3] <- cut(
      todaystatus.df[, 2],
      breaks = c(0, 120, 130, 1000),
      labels = c(
        ", transmission is unlikely",
        ", the transmission season may be starting soon if there is more warm weather",
        ", transmission is possible, consult your veterinarian"
      )
    )
    t <-
      paste((todaystatus.df[nrow(todaystatus.df), 2]), "HDUs", todaystatus.df[nrow(todaystatus.df), 3], sep =
              "")
    t <- as.character(t)
    t
  ...
  output$cutofftable <- renderDataTable(cutoffdata(), options = list(
    dom = 't', 
    paging = FALSE, 
    rownames = FALSE, 
    pageLength = 50,
  ...
  cutoffdata <- reactive({
    ...
    for (n in 1:nrow(status.df)) {
      status.df[n, 4] <-
        ifelse(status.df[(n + 1), 2] < 130 & status.df[n, 2] > 130, 1, 0)
    }
    ...
    validate(need(
      length(a) > 0,
      message = paste(
        "If this message is being shown, there are no cutoff dates. All dates of each year since 2015 have the same status"
      )
    ))
    ...
    colnames(df1) <-
      c(
        "Date",
        paste("Status for ", input$postcode, sep = ""),
    ...
    cutoff.df <- data.frame(cutoff.df[order(as.Date(cutoff.df[, 1], format = "%Y-%m-%d")), ], row.names = NULL)
    cutoff.df <- cutoff.df[rev(seq_len(nrow(cutoff.df))), ]    
    colnames(cutoff.df) <-c("Date", paste("Status for ", input$postcode, sep = ""), "Consecutive days since last transmission")
    cutoff.df
  })
  ...
  percentagetabledata <- reactive({
    ...
    for (n in 1:nrow(perc.df)) {
      perc.df[n, 4] <-
        ifelse(perc.df[(n + 1), 2] < 130 & perc.df[n, 2] > 130, 1, 0)
    ...
    validate(need(
      length(a) > 0,
      message = paste(
        "If this message is being shown, it indicates 100% of each year has the same status"
      )
    ))
    ...
    for (i in 1:length(yseq.df[, 1])) {
      ...
      percentagetabledata[i, 2] <-
        (yseq.df[i, 2] - sum(tdf[, 3])) / yseq.df[i, 2] * 100
      
    }
    
    percentagetabledata[, 2] <- as.numeric(percentagetabledata[, 2])
    percentagetabledata[, 2] <-
      round(percentagetabledata[, 2], digits = 2)
    
    percentagetabledata[nrow(percentagetabledata), 2] <-
      ifelse(yseq.df[nrow(yseq.df), 2] >= 365, percentagetabledata[nrow(percentagetabledata), 2], NA)
    
    colnames(percentagetabledata) <-
      c("Year", "Percentage of the year at risk")
    percentagetabledata
